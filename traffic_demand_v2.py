"""
traffic_demand_v2.py — Research-backed, competition-grade pipeline
====================================================================
Implements ALL findings from the deep research:
  ...
  7. CatBoost with native categoricals   -- ordered boosting, most diverse model
  8. LightGBM DART                       -- dropout reduces overfitting on small data
...
# ── Try CatBoost, gracefully skip if not installed ───────────────────────────
try:
...
def attach_reference_features(
    ...
    rank_df = (
        day48.groupby(["RoadType", "minute"])
        .apply(lambda g: g.assign(geo_rank_in_road=g["demand"].rank(pct=True)))
        .reset_index(drop=True)[["geohash", "minute", "RoadType", "geo_rank_in_road"]]
    )
    ...
    k49_sorted = k49.sort_values(["geohash", "minute"])
    roll_stats = {}
    for window, label in [(4, "1h"), (8, "2h")]:
        roll_stats[label] = (
            k49_sorted.groupby("geohash")["demand"]
            .apply(lambda x: x.rolling(window, min_periods=1).mean())
            .reset_index(level=0, drop=True)
        )
    k49_aug = k49.copy()
    for label, stat in roll_stats.items():
        k49_aug[f"roll_{label}"] = stat.values
    ...
    if len(k49) > 0:
        ...
        dod_geo = joined_dod.groupby("geohash").apply(
            lambda g: (g["demand"].sum() + 0.5 * global_dod) / (g["d48_dod"].sum() + 0.5)
        ).rename("dod_ratio_geo")
        # GH5 prefix DoD ratio
        joined_dod["gh5"] = joined_dod["geohash"].str[:5]
        dod_gh5 = joined_dod.groupby("gh5").apply(
            lambda g: (g["demand"].sum() + 1.0 * global_dod) / (g["d48_dod"].sum() + 1.0)
        ).rename("dod_ratio_gh5")
        # Road-type DoD ratio
        joined_dod["RoadType"] = joined_dod["geohash"].map(
            k49.set_index("geohash")["RoadType"].to_dict()
        )
        dod_road = joined_dod.groupby("RoadType").apply(
            lambda g: (g["demand"].sum() + 2.0 * global_dod) / (g["d48_dod"].sum() + 2.0)
        ).rename("dod_ratio_road")
...
def make_extratrees_pipeline(numeric: list[str], categorical: list[str]):
    ...
    et = ExtraTreesRegressor(
        n_estimators=300,       # Increased from 110
        min_samples_leaf=1,     # Fine-grained (research suggests 1 for this size)
        max_features=0.72,      # Slightly reduced for more diversity
        n_jobs=-1,
...
def train_and_predict(
    ...
    # CatBoost (with native categoricals — ordered boosting)
    if HAS_CATBOOST:
...
def fit_ridge_meta(oof_preds: pd.DataFrame, y: np.ndarray, road_types: pd.Series):
    """Ridge meta-learner (alpha=5) — prevents overfitting the validation folds."""
    ...
    if "extra_trees" in cols:
        ...
        print(f"  ExtraTrees OOF R²: {et_score:.5f} | Ridge blend R²: {blend_score:.5f}")
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 9 — RESIDUAL BIAS CORRECTION (novel post-processing)
# ══════════════════════════════════════════════════════════════════════════════
...
def compute_residual_bias(
    ...
    """
    For each geohash, compute the mean model error on the KNOWN early Day-49 slots.
    This per-geohash systematic bias likely persists into the test window.
    """
    residuals = known49_df["demand"].to_numpy() - model_preds_on_known49
    bias_series = pd.Series(residuals, index=known49_df.index)
    geo_bias = (
...
def run(data_dir: Path = Path("dataset")) -> None:
    print("=" * 60)
    print("Traffic Demand V2 — Research-Backed Pipeline")
    print("=" * 60)
    ...
    print("\n  Individual model OOF R² (with road bounds):")
    ...
    print(f"\n  Final blend OOF R² (with road bounds): {blend_score:.5f}")
    ...
    print("\n" + "=" * 60)
    print(f"  OOF Validation R²: {blend_score:.5f} → Score: {100*blend_score:.2f}")
